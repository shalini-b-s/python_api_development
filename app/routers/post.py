...
#         password = '0322', connection_factory= RealDictConnection)
#         cursor = conn.cursor()
...
def update_post(id: int, post: schema.PostCreate,  db: Session = Depends(get_db), current_user: int = Depends(oauth.get_current_user)):
    ...
    # cursor.execute("""update posts set title = %s, content = %s, published = %s returning * where id = %s""", (post.title, post.content, post.published, str(id)))
